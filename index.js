    ...
    #!/bin/bash
      {
        ...
        if [ -n "apex_classes" ]; then
          echo "\t<types>\n\t\t<members>\n$(git diff --ignore-all-space --ignore-blank-lines --ignore-cr-at-eol $(git log --reverse --since="${sinceCommit}" | head -n 1 | cut -d " " -f 2) ^HEAD | grep .*\.cls$ | cut -d / -f 6 | cut -d " " -f 1 | sort | uniq | sed 's/^/\t\t\t/')\n\t\t</members>\n\t\t<name>ApexClass</name>\n\t</types>";
        fi
        
        if [ -n "aura_bundle" ]; then
          echo "\t<types>\n\t\t<members>\n$(git diff --ignore-all-space --ignore-blank-lines --ignore-cr-at-eol $(git log --reverse --since="${sinceCommit}" | head -n 1 | cut -d " " -f 2) ^HEAD | grep .*\.js$ | cut -d / -f 6 | cut -d " " -f 1 | sort | uniq | xargs | sed 's/^/\t\t\t/')\n\t\t</members>\n\t\t<name>AuraDefinitionBundle</name>\n\t</types>";
        fi
